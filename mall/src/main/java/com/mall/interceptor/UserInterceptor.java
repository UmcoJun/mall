...
public class UserInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		// 클라이언트의 요청이 인증이 되었는지 여부를 체크(세션객체를 확인)
		...
		if(user == null) { // 인증정보가 존재하지 않을 때
			...
			// 클라이언트 요청이 ajax(비동기)요청인지 동기요청인지 체크.
			if(isAjaxRequest(request)) { // ajax요청인 경우
				
				// ajax요청은 콜백기능을 가지고 있어, 다시 클라이언트의 콜백으로 제어가 동작되어지는 특징을 가지고 있어
				// 스프링의 작업이 진행되지 않는다.
				//response.sendRedirect("/member/login"); 
				System.out.println("Ajax요청");
				response.sendError(400);
			}else { // 동기요청인 경우. 로그인이 필요한 주소를 요청
	...
	private void getDestination(HttpServletRequest request) {
		...
		String uri = request.getRequestURI();  // 목적지 주소 예) /cart/cart_list
		String query = request.getQueryString(); // ?뒤에 파라미터=값 예) pdt_num=10
