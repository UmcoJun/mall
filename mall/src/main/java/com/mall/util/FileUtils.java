...
public class FileUtils {
	...
	public static String uploadFile(String uploadFolder, String uploadDateFolderPath, MultipartFile uploadFile) {
		...
		if(uploadPath.exists() == false) {
			uploadPath.mkdirs(); // 존재하지않으면 하위(자손)폴더까지 한번에 생성
		}
		...
		try {
			...
			if(checkImageType(saveFile)) {
				...
				// 파일생성위치 : D:\\IT\\upload\\goods\\2022\\11\\22\\abc.gif
	...
	// 날짜폴더 생성 및 운영체제 경로의 구분자에 맞게 리턴.
	public static String getFolder() {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		
		Date date = new Date();
		
		String str = sdf.format(date); // "2022-11-23" 실시간날짜
	...
	public static ResponseEntity<byte[]> getFile(String uploadPath, String fileName) throws IOException{
		...
		headers.add("ContentE-Type", Files.probeContentType(file.toPath()));
	...
	// 파일삭제
	// uploadPath : "D:\\IT\\upload\\goods\\"
	// fileName : 2022/11/23/,		c5ba18eb-2868-41c5-aca5-b24bec3ff280_닭가슴살소시지.jpg
