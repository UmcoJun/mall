			...
			<div class="box-body no-padding">
				...
				<tbody><tr>
					...
					<tr>
				      ...
				      <td>￦<span class="pdt_price"><fmt:formatNumber type="currency" pattern="#,###" currencySymbol="₩" value="${cartListVO.pdt_price }" />
				      </span></td>
				      <td>
				      ...
				      <td>택배</td>
				      <td>무료</td>
				      <td>￦<span class="sales_price"><fmt:formatNumber type="currency" pattern="#,###" currencySymbol="₩" value="${cartListVO.sales_price }" />
				      </span></td>
				      <td>
				      	<input type="hidden" name="cart_code" value="${cartListVO.cart_code }">
				      	<button type="button" name="btnCartDel" class="btn btn-link">삭제</button>
				      </td>
					</tr>
				...
				<tfoot>
					...
					<c:if test="${empty cartList }">
						<td colspan="10" style="text-align: center">상품을 담아주세요.</td>
					</c:if>
					<c:if test="${!empty cartList }">
						<td colspan="10" style="text-align: right">총금액: ￦<span id="sum_price"><fmt:formatNumber type="currency" pattern="#,###" currencySymbol="₩" value="${sum_price }" /></span></td>
					</c:if>
					</tr>
				</tfoot>
				</table>
  ...
  <script>
    ...
    $(document).ready(function() {
      ...
      $("button[name='btnQtyChange']").on("click", function() {
        ...
        let btnQtyChange = $(this); // 변경 클릭 태그
      ...
      $.ajax({
        ...
        success:  function(result) {
          if(result == "success") {
            ...
            btnQtyChange.parent().parent().find("span.sales_price").text($.numberWithCommas(pdt_price * cart_amount)); // 화면표시는 콤라를 찍기
      ...
      $("button[name='btnCartDel']").on("click", function() {
        ...
        let current_tr = $(this).parent().parent(); // ajax구문이 실행되기전 삭제행을 참조(작성)해야한다.
    ...
    function fn_sum_price() {
        ...
        let sum_price = 0;
          $("table#cartlist span.sales_price").each(function(index, item) {
            sum_price += parseInt($.withoutCommas($(item).text())); // 연산하는 경우는 콤마를 제거
          });
            
          $("span#sum_price").text($.numberWithCommas(sum_price)); // 화면표시는 콤라를 찍기
          }
